...
export const getUserData = async(req,res)=>{
    try{
        ...
        if(!user){
            return res.json({success:false,mongoose:"User Not Found"})
        }
...
try{
    ...
    const line_items=[{
     ...
     const session = await stripeInstance.checkout.sessions.create({
         ...
         mode: "payment",
         metadata: {
             purchaseId: newPurchase._id.toString() // Ensure this is correct
         }
     });
...
const user = await User.findById(userId);
if(!user || !user.emrolledCourses.included(courseId)){
